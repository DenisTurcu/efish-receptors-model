...
class LfpResponseDataset(Dataset):
    ...
    def __getitem__(self, index):
        return (
            self.stimuli[index].reshape(1, -1),
            self.base_stimuli[index].reshape(1, -1),
            self.response[index].reshape(-1),
        )
    ...
    def _process_dataframe(self):
        ...
        if self.mean_response:
            df = df.groupby("stimulus_marker").apply(
                lambda x: x[["waveform", "mean_lfp_response_modulation", "base_waveform"]].iloc[0]
            )
            self.stimuli = np.vstack(df["waveform"]).astype(np.float32)  # type: ignore
            self.base_stimuli = np.vstack(df["base_waveform"]).astype(np.float32)  # type: ignore
